...
const double max_angle = 30.0 * M_PI / 180.0;  // 30 degrees in radians
...
public:
  ...
  : Node("follow_velocity")
  {
    ...
    kinematics_ = std::make_shared<antsy_kinematics::Kinematics>(
      ...
      // std::vector<std::string>{"foot_fl_1", "foot_ml_1", "foot_bl_1", "foot_br_1", "foot_mr_1", "foot_fr_1"});
...
private:
  ...
  KDL::Frame constrainFootOrientation(const KDL::Frame& frame)
  {
    // Extract the current orientation
    KDL::Rotation current_rotation = frame.M;

    // Calculate the roll, pitch, and yaw angles of the current orientation
    double roll, pitch, yaw;
    current_rotation.GetRPY(roll, pitch, yaw);

    // Constrain the roll and pitch to be within the tolerance
    roll = std::clamp(roll, -max_angle, max_angle);
    pitch = std::clamp(pitch, -max_angle, max_angle);

    // Create the constrained rotation
    KDL::Rotation constrained_rotation = KDL::Rotation::RPY(roll, pitch, yaw);

    // Return the frame with the constrained orientation
  ...
  void timerCallback()
{
  ...
  // Get minimum duration until any of the supporting legs transitions to RISING
  ...
  for (int i = 0; i < nb_legs_; i++) {
    ...
    foot_frame = constrainFootOrientation(foot_frame);  // Apply orientation constraint
